...
RESPONSE_SCHEMA_MAX_LENGTH = 500000


CODE_PARSING_SCHEMA_TEMPLATE = """Here is an API response schema from an OAS and a query. 
Use the API response as you receive it. Do not create one by yourself!!!
...
Please filter the results based on the user query. 
...
if you have been asked to get a management device, take from the response only those devices that their virtual_type is equal to management 
Always use the device id if it exist
...
"The release date of the album is 2002-11-03"
"The id of the person is 12345"
Note you should generate only Python code and you should use the json Python library to parse the JSON response into a Python json object
DO NOT use fields that are not in the response schema.
If you have been asked to get a management device filter the response by field named virtual_type with value that is management 
If you have been asked if a traffic between a source and destination on some service is allowed filter the response by field named traffic_allowed
If you have been asked to get a created ticket id, response back that the id is not provided in the response
...
class ResponseParser(Chain):
    ...
    max_json_length_1: int = 500000
    max_json_length_2: int = 200000
